...
def test_merge_does_not_special_case_any_currency():
    ...
    a = e.empty_filing(); a["metadata"]["currency"] = "USD"
    b = e.empty_filing(); b["metadata"]["currency"] = None
...
def test_system_prompt_uses_profile_jurisdiction_label():
    fake_profile = {"ticker": "ABCD", "jurisdiction": "United Arab Emirates",
                    "company_name": "Acme Co.", "as_of_year": 2024,
                    "sub_sector": "industrial", "reporting_currency": "AED",
                    "framework_as_of": "IFRS", "active_events": [],
                    "active_subsidiaries": [], "watch_kpis": [],
...
def test_profile_context_renders_unknown_jurisdiction_safely():
    ...
    """
    p = {"ticker": "XYZ", "name_as_of": "Acme Group", "as_of_year": 2024,
         "reporting_currency": "EUR", "framework_as_of": "IFRS",
         "active_events": [], "active_subsidiaries": [], "watch_kpis": [],
         "segments_expected": {"by_geography": [], "by_business": []},
         "accounting_quirks": []}
...
def test_cli_currency_and_framework_overlay_fill_blank_metadata(monkeypatch):
    ...
    ns = argparse.Namespace(
        no_llm=True, guided=True, guided_notes=True,
        symbol="ABCD", sector="industrial", year=2024, period="FY",
        currency="AED", framework="AAOIFI", jurisdiction="qatar",
        pages_per_chunk=12, overlap=1, _profile=None,
    )
...
def _fake_loader():
    ...
    return profiles.JurisdictionLoader(
        build_profile=lambda t: {"ticker": t, "jurisdiction": "United Arab Emirates",
                                  ...
                                  "watch_kpis": ["KPI_NPM"], "names": [],
                                  "segments_expected": {"by_geography": ["UAE"], "by_business": []},
                                  "subsidiaries": [{"name": t, "country": "AE",
                                                     "currency": "AED", "from": None, "to": None}],
                                  "events": [], "peers": [], "accounting_quirks": [],
                                  "fiscal_year_end": "12-31"} if t == "ALDAR" else None,
