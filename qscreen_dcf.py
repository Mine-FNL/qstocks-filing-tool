...
def _check(discount_rate: float, terminal_growth: float) -> None:
    ...
    if terminal_growth >= discount_rate:
        raise ValueError(
            "terminal_growth must be < discount_rate (Gordon terminal diverges otherwise)"
        )
    ...
    *,
    ...
    if archetype in ("conventional_bank", "islamic_bank", "insurance"):
        ...
        result = residual_income(
            bv, roe, discount_rate=r, growth=g, terminal_growth=gt, years=years, shares=shares
        )
    else:
        ...
        if fcf is None:
            return {
                ...
                "warnings": [
                    *warnings,
                    "need free cash flow (CF_FCF or CF_OCF+CF_CAPEX) for a DCF",
                ],
            }
        model_fn, base_kwargs = (
            fcfe_dcf,
            {"base_fcf": fcf, "terminal_growth": gt, "years": years, "shares": shares},
        )
        result = fcfe_dcf(
            fcf, discount_rate=r, growth=g, terminal_growth=gt, years=years, shares=shares
        )
...
def main() -> int:
    ...
    up = (
        f", upside {out['upside'] * 100:+.0f}% vs {out['price']}"
        if out.get("upside") is not None
        else ""
    )
    print(
        f"💰 {out['symbol']} [{v['model']}] → {path}  ⇒ {ps}{up}  "
