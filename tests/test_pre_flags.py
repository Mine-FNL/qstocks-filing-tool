...
def _li(code: str, value, comparatives=None):
    return {"account_code": code, "value": value,
            "comparatives": comparatives or [],
            "depth": 0, "is_subtotal": False}


def _filing(meta=None, statements=None, audit=None, notes=None):
    return {"metadata": meta or {}, "statements": statements or [],
            "audit": audit or {}, "notes": notes or []}
...
@pytest.mark.parametrize("ticker,year", [
...
def test_issuer_facts_surface_for_known_tickers(ticker, year):
    f = _filing(meta={"symbol": ticker, "fiscal_year": year,
                       "sector": "conventional_bank", "fiscal_period": "FY"})
...
def test_going_concern_flag_emitted_when_present():
    f = _filing(meta={"symbol": "QGMD", "fiscal_year": 2021, "fiscal_period": "FY"},
                 audit={"opinion_type": "qualified",
                         "material_uncertainty_going_concern":
                           {"present": True, "text": "Material uncertainty …"}})
    flags = pre_flags.run_pre_flags(f)
...
def test_qualified_insurer_with_reserves_flag():
    f = _filing(
        meta={"symbol": "DOHI", "sector": "insurance", "fiscal_year": 2022,
              "fiscal_period": "FY"},
        audit={"opinion_type": "qualified",
                "key_audit_matters": [
...
def test_afs_dominates_ni_when_large_ratio():
    f = _filing(meta={"symbol": "QATI", "sector": "insurance",
                       "fiscal_year": 2022, "fiscal_period": "FY"},
                 statements=[{"type": "balance_sheet", "title": "BS",
                                "period_label": "FY",
                                "line_items": [
...
def test_dual_income_statement_takaful():
    f = _filing(
        meta={"symbol": "QISI", "sector": "insurance", "sub_sector": "Takaful Insurance",
              "fiscal_year": 2022, "fiscal_period": "FY"},
        statements=[{"type": "income_statement", "title": "Policyholders IS",
                       "period_label": "FY", "line_items": []},
                      {"type": "income_statement", "title": "Shareholders IS",
                       "period_label": "FY", "line_items": []}])
    flags = pre_flags.run_pre_flags(f)
    rules = {x.rule_id for x in flags}
...
def test_merge_into_filing_idempotent():
    f = _filing(meta={"symbol": "ZHCD", "fiscal_year": 2020,
                       "fiscal_period": "FY"})
    flags = pre_flags.run_pre_flags(f)
